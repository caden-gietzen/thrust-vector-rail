...
% Set TIER to select the operating-point strategy:
%
%   1 — Fixed-gain LQR : single linearization at theta*=0, T*=T_r_nom
%   2 — Angle-scheduled: sweep theta* grid at nominal thrust
%   3 — Pair-scheduled  : sweep (theta*, T*) grid  [not yet implemented]
%
% States:  x = [p; v; theta; T]      (m, m/s, rad, N)
% Inputs:  u = [theta_cmd_rad; T_cmd_N]
%
% Output:  gain_schedule_tier{N}.mat
...
if TIER == 1
    p_max        = 0.001;         % m    position error
    v_max        = 0.005;         % m/s  velocity
    theta_max    = deg2rad(60);   % rad  servo angle deviation from trim
    T_max_dev    = 0.25;           % N    thrust deviation from trim
    du_theta_max = deg2rad(180);  % rad  max servo command; must exceed arcsin(mu_c/T*) ≈ 18.5° to overcome friction
    dT_max       = 0.25;           % N    max thrust command deviation

elseif TIER == 2
    p_max        = 0.001;          % m
    v_max        = 0.005;          % m/s
    theta_max    = deg2rad(60);     % rad  — tightened so Q_theta ≈ Q_p; forces theta to zero at same rate as position
    T_max_dev    = 0.25;           % N
    du_theta_max = deg2rad(180);   % rad
    dT_max       = 0.25;           % N

elseif TIER == 3
    p_max        = 0.001;          % m
    v_max        = 0.005;          % m/s
    theta_max    = deg2rad(60);   % rad
    T_max_dev    = 0.25;           % N
    du_theta_max = deg2rad(180);   % rad
    dT_max       = 0.25;           % N
end

Q = diag([1/p_max^2, 1/v_max^2, 1/theta_max^2, 1/T_max_dev^2]);
R = diag([1/du_theta_max^2, 1/dT_max^2]);
...
mat_path = fullfile(here, sprintf('gain_schedule_tier%d.mat', TIER));
...
if TIER == 1
    ...
    save(mat_path, 'K_fixed', 'x_star', 'u_star', 'A', 'B', 'Q', 'R');
