% build_model.m
%
% Programmatically creates a Simulink model for the selected LQR tier.
% Run trim_analysis.m with the matching TIER first to generate the .mat file,
% then run this script once. Open the resulting .slx to simulate.
%
%   TIER 1 → fixed_lqr.slx         (fixed-gain, no scheduling input)
%   TIER 2 → theta_sched_lqr.slx   (gain scheduled on theta_cmd)
%   TIER 3 → pair_sched_lqr.slx    (gain scheduled on theta_cmd + T_cmd) [stub]
%
% Model structure (Tier 2 shown; Tier 1 omits theta_cmd block):
%
%   theta_cmd ──────────────────────────────────────────────────────────────┐
%                                                                            │
...
switch TIER
    case 1
        model_name = 'fixed_lqr';
        mat_file   = 'gain_schedule_tier1.mat';
        ic_str     = '[0.15; 0; 0; 2.574]';   % displaced position, at rest, at theta*=0 trim
    case 2
        model_name = 'theta_sched_lqr';
        mat_file   = 'gain_schedule_tier2.mat';
        ic_str     = '[0.15; 0; pi/6; 2.574]'; % displaced, at rest, angled, nominal thrust
    case 3
        model_name = 'pair_sched_lqr';
        mat_file   = 'gain_schedule_tier3.mat';
        ic_str     = '[0.15; 0; pi/6; 2.574]'; % TBD — at rest, nominal thrust (matches Tier 2 convention)
    otherwise
        error('Unknown TIER: %d. Must be 1, 2, or 3.', TIER);
end
...
if ~isfile(mat_path)
    error('%s not found — run trim_analysis.m with TIER=%d first.', mat_file, TIER);
end
...
add_block('simulink/Sources/Constant', [model_name '/p_ref'], ...
    'Position', [625 375 685 405], 'Value', '0');
...
%% Wire — Tier-Specific
if TIER == 1
    % Controller takes only x (no scheduling input)
    ...
    % Theta subplot: actual theta only (no reference overlay)
    lh = add_line(model_name, 'Demux_x/3', 'Scope_states/3', 'autorouting', 'smart');
    set_param(lh, 'Name', 'theta (rad)');

elseif TIER == 2
    % Scheduling variable = live theta (x(3)), wired as separate controller port
    add_line(model_name, 'integrator/1', 'controller/1', 'autorouting', 'smart');
...
elseif TIER == 3
    % Scheduling variables = live theta (x(3)) and T (x(4)), extracted from x inside controller
    add_line(model_name, 'integrator/1', 'controller/1', 'autorouting', 'smart');
    add_line(model_name, 'controller/1', 'plant/2',      'autorouting', 'smart');
    add_line(model_name, 'integrator/1', 'plant/1',      'autorouting', 'smart');
    add_line(model_name, 'plant/1',      'integrator/1', 'autorouting', 'smart');
...
lines = {
    ...
    '% Control law: u = u_star - K_fixed * (x - x_star)'
...
lines = {
    ...
    '% Control law: u = u_star - K * (x - x_star), x_star = [0;0;0;T_nom]'
    ...
    '    end'
    'end'
    'T_nom  = 2.574;                 % nominal thrust (1700 µs; same as Tier 1)'
    'x_star = [0; 0; 0; T_nom];      % all states target zero; T holds at nominal'
    'u_star = [0; T_nom];            % feedforward: zero servo angle, nominal thrust'
    'u = u_star - K * (x - x_star);'
    'u(1) = max(min(u(1), pi/3), -pi/3);  % servo ±60°'
    'u(2) = max(min(u(2), 4.17), 0);      % thrust 0–4.17 N (static sweep at 1950 µs)'
    'end'
...
lines = {
    'function u = controller(x)'
    '% Pair-scheduled LQR. K(theta,T) from 2D gain table; scheduling vars extracted from x.'
    '% theta = x(3), T_cur = x(4); target is always x_star = [0;0;0;T_nom].'
    ...
    'theta   = x(3);'
    'T_cur   = x(4);'
    'theta_q = max(min(theta, theta_s(end)), theta_s(1));'
    'T_q     = max(min(T_cur, T_s(end)),     T_s(1));'
    'K = zeros(2, 4);'
    'for i = 1:2'
    '    for j = 1:4'
    '        K_mat = reshape(K_s(i,j,:,:), [numel(theta_s), numel(T_s)]);'
    '        K_at_T = zeros(numel(T_s), 1);'
    '        for k = 1:numel(T_s)'
    '            K_at_T(k) = interp1(theta_s(:), K_mat(:,k), theta_q, ''linear'', ''extrap'');'
    '        end'
    '        K(i,j) = interp1(T_s(:), K_at_T, T_q, ''linear'', ''extrap'');'
    '    end'
    'end'
    'T_nom  = 2.574;'
    'x_star = [0; 0; 0; T_nom];'
